from datetime import datetime, timezone as TimeZone, timedelta
import unittest
...
class CandlefeedUnitTest(unittest.TestCase):
  ...
  def test_candle_indexer(self):
    ...
    candle_indexer = CandleIndexer(indexs[0], 
...
class PandasCandleFeedUnitTest(unittest.TestCase):
  ...
  def test_load_data_on_level_sec1_min1(self):
    _sol_candle_feed = PandasCandleFeed(trading_pair=common_trading_pair.SOLUSDTP,
                                  timeframe_level=TimeframeType.sec1,
  ...
  def test_load_data_on_level_sec1(self):
    ...
    #测试秒级数据是否匹配
    sec = sol_candle_feed.get_ohlcv(timeframe=TimeframeType.sec1, position=indexs[0])
    ...
    #测试秒级数据是否偏移是否正常
    sec_offset = sol_candle_feed.get_ohlcv(timeframe=TimeframeType.sec1, position=indexs[0], offset=1)
    ...
    #测试分钟级数据是否匹配
    print("测试分钟数据")
    print(f"测试时间： 时间戳： {indexs[0]} \niso:{datetime.fromtimestamp(indexs[0], tz=TimeZone(timedelta(hours=0))).isoformat()}")
    min1 = sol_candle_feed.get_ohlcv(indexs[0], timeframe=TimeframeType.min1)
    self.assertEqual(min1['volume'], sol_usdt_1m.loc[indexs[0], 'volume'])
    self.assertEqual(min1['open'], sol_usdt_1m.loc[indexs[0], 'open'])
    self.assertEqual(min1['high'], sol_usdt_1m.loc[indexs[0], 'high'])
    self.assertEqual(min1['low'], sol_usdt_1m.loc[indexs[0], 'low'])
    self.assertEqual(min1['close'], sol_usdt_1m.loc[indexs[0], 'close'])

    print(sol_usdt_1s_dataframe.loc[indexs[0]:indexs[5]])
    for i in range(0, 6):
      sol_candle_feed.update(indexs[0] + i)

    
    #测试分钟数据与秒级的聚合是否正确
    min1_sec05 = sol_candle_feed.get_ohlcv(indexs[5], timeframe=TimeframeType.min1)
    ...
    #测试分钟数据是否偏移是否正常
    min1_offset = sol_candle_feed.get_ohlcv(indexs[0], timeframe=TimeframeType.min1, offset=1)
    ...
    #测试5分钟数据是否匹配
    min5 = sol_candle_feed.get_ohlcv(indexs[0], timeframe=TimeframeType.min5)
    self.assertEqual(min5['volume'], sol_usdt_5m.loc[indexs[0], 'volume'])
    self.assertEqual(min5['open'], sol_usdt_5m.loc[indexs[0], 'open'])
    self.assertEqual(min5['high'], sol_usdt_5m.loc[indexs[0], 'high'])
    self.assertEqual(min5['low'], sol_usdt_5m.loc[indexs[0], 'low'])
    self.assertEqual(min5['close'], sol_usdt_5m.loc[indexs[0], 'close'])

    #测试5分钟数据是否偏移是否正常
    min5_offset = sol_candle_feed.get_ohlcv(indexs[0], timeframe=TimeframeType.min5, offset=1)
    ...
    #测试15分钟数据是否匹配
    min15 = sol_candle_feed.get_ohlcv(indexs[0], timeframe=TimeframeType.min15)
    self.assertEqual(min15['volume'], sol_usdt_15m.loc[indexs[0], 'volume'])
    self.assertEqual(min15['open'], sol_usdt_15m.loc[indexs[0], 'open'])
    self.assertEqual(min15['high'], sol_usdt_15m.loc[indexs[0], 'high'])
    self.assertEqual(min15['low'], sol_usdt_15m.loc[indexs[0], 'low'])
    self.assertEqual(min15['close'], sol_usdt_15m.loc[indexs[0], 'close'])

    #测试15分钟数据是否偏移是否正常
    min15_offset = sol_candle_feed.get_ohlcv(indexs[0], timeframe=TimeframeType.min15, offset=1)
    ...
    #测试1小时数据是否匹配
    h1 = sol_candle_feed.get_ohlcv(indexs[0], timeframe=TimeframeType.h1)
    self.assertEqual(h1['volume'], sol_usdt_1h.loc[indexs[0], 'volume'])
    self.assertEqual(h1['open'], sol_usdt_1h.loc[indexs[0], 'open'])
    self.assertEqual(h1['high'], sol_usdt_1h.loc[indexs[0], 'high'])
    self.assertEqual(h1['low'], sol_usdt_1h.loc[indexs[0], 'low'])
    self.assertEqual(h1['close'], sol_usdt_1h.loc[indexs[0], 'close'])

    #测试1小时数据是否偏移是否正常
    h1_offset = sol_candle_feed.get_ohlcv(indexs[0], timeframe=TimeframeType.h1, offset=1)
    ...
    #测试4小时数据是否匹配
    h4 = sol_candle_feed.get_ohlcv(indexs[0], timeframe=TimeframeType.h4)
    self.assertEqual(h4['volume'], sol_usdt_4h.loc[indexs[0], 'volume'])
    self.assertEqual(h4['open'], sol_usdt_4h.loc[indexs[0], 'open'])
    self.assertEqual(h4['high'], sol_usdt_4h.loc[indexs[0], 'high'])
    self.assertEqual(h4['low'], sol_usdt_4h.loc[indexs[0], 'low'])
    self.assertEqual(h4['close'], sol_usdt_4h.loc[indexs[0], 'close'])

    #测试4小时数据是否偏移是否正常
    h4_offset = sol_candle_feed.get_ohlcv(indexs[0], timeframe=TimeframeType.h4, offset=1)
    ...
    #测试1天数据是否匹配
    d1 = sol_candle_feed.get_ohlcv(indexs[0], timeframe=TimeframeType.d1)
    self.assertEqual(d1['volume'], sol_usdt_1d.loc[indexs[0], 'volume'])
    self.assertEqual(d1['open'], sol_usdt_1d.loc[indexs[0], 'open'])
    self.assertEqual(d1['high'], sol_usdt_1d.loc[indexs[0], 'high'])
    self.assertEqual(d1['low'], sol_usdt_1d.loc[indexs[0], 'low'])
    self.assertEqual(d1['close'], sol_usdt_1d.loc[indexs[0], 'close'])

    #测试1天数据是否偏移是否正常
    d1_offset =  sol_candle_feed.get_ohlcv(indexs[0], timeframe=TimeframeType.d1, offset=1)
  ...
  #   base_pos = indexs[0]
  #   # 1天匹配
  #   d1 =candle_feed.get_ohlcv(base_pos, timeframe=TimeframeType.d1)
  #   self.assertEqual(d1['open'], sol_usdt_1d.loc[base_pos, 'open'])
  #   # 1天偏移
  #   d1_offset = candle_feed.get_ohlcv(base_pos, timeframe=TimeframeType.d1, offset=1)
  #   offd1 = base_pos + MILLISECONDS_IN_A_DAY
  #   self.assertEqual(d1_offset['open'], sol_usdt_1d.loc[offd1, 'open'])
