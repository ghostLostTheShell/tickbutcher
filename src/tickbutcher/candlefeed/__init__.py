...
class CandleIndexer:
  position: int
  candleFeed: Optional[CandleFeed]
  timeframe: Optional[TimeframeType]
  ...
  def __getattr__(self, name: str):
    ...
    self.candlefeed = self.trading_pair_candle_table.get(tp)
    if self.candlefeed is None:
      raise ValueError(f"No candle feed available for trading pair id: {financial_type_id}")  

    return self

  def __getitem__(self, key: Tuple[int , Optional[str]] | int)->Series:
